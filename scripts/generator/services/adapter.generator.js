...
const SERVICE_FOLDER = path.join(
  process.cwd(),
  'src/modules/libs/%LIB_NAME%/src/infrastructure/adapter/%SUB_FOLDER%',
);
...
const generateAdapter = (library_name, sub_folder, service_name) => {
  ...
  const newServicePath = `${path.join(realPath, service_name_lowercase)}.adapter.ts`;
